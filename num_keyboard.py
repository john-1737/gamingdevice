...
def int_keyboard(s, startnum='', inittext=''):
    ...
    while True:
        ...
        if startnum == '':
        ...
        elif textwidth+spacewidth <= width/4*3:
            ...
            render_text(startnum[:text_cursor], (width/4*3-font.render('|', True, (255, 255, 255)).get_rect().width, 0), font=font, lanchor=True)
...
'''.splitlines()]
    images = [[None for i in range(8)], [cross, square] + [None for i in range(6)]]
    sleep(0.2)
    while True:
        for event in pg.event.get():
            if event.type == QUIT:
